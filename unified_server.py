...
class UnifiedHandler(SimpleHTTPRequestHandler):
    ...
    def __init__(self, *args, **kwargs):
        # 设置静态文件目录
        self.directory = '/root/.openclaw/workspace/academic-color-palette'
        super().__init__(*args, directory=self.directory, **kwargs)

    def log_message(self, format, *args):
        """自定义日志"""
        print(f"[Unified Server] {args[0]}")
        sys.stdout.flush()
    ...
    def handle_ai_request(self):
        ...
        try:
            ...
            print(f"收到AI请求: {user_message[:50]}...")
            sys.stdout.flush()
...
def run_server(port=8888):
    ...
    print(f"🎨 统一服务器运行在端口 {port}")
    print(f"📍 静态文件: http://localhost:{port}/")
    print(f"📍 AI API: http://localhost:{port}/api/ai-consult")
    sys.stdout.flush()
    try:
        httpd.serve_forever()
